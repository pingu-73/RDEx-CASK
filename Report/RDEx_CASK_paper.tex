...
% The reference implementation is \texttt{rdex\_exp217.cpp}. Configuration values are
% \begin{align*}
% H &= 10, &\quad SG &= 180, &\quad p_{arch}^{floor} &= 0.65, \\
% CR_{sat} &= 0.95, &\quad SR_{th} &= 0.10, &\quad \rho_{EB}^{(0)} &= 0.5, \\
% |\mathcal{A}|_{\max} &= 50, &\quad N_0 &= 10D, &\quad N_{\min} &= 4.
% \end{align*}
% The seed protocol uses a wall-clock seed by default \&a CLI-provided seed for reproducibility. Each $(\text{run}, \text{function})$ pair is reseeded as $\text{base\_seed} + 1000\cdot\text{run} + 7\cdot\text{func}$. Per-run runtime is unchanged within noise compared with RDEx.
...
\begin{table*}[!t]
\centering
\caption{Time-to-target (TTT) on the 28 CEC 2025 CSOP functions, $D = 30$, 25 runs per function. RDEx, UDE-III, \& CL-SRDE values are reproduced from Table VI of \cite{tao2026rdex}. $2001$ means the target was never reached.}
\label{tab:ttt}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{rcccc}
...
\bibitem{tao2026rdex}
S. Tao, Y. Yang, R. Zhao, K. Wang, S. Liu, \&S. Gao,
...
in \emph{Proc. IEEE Congress on Evolutionary Computation}, 2014, pp. 1658--1665.

\bibitem{5208221}
J. Zhang \&A. C. Sanderson,
``JADE: Adaptive differential evolution with optional external archive,''
\emph{IEEE Trans. Evol. Comput.}, vol. 13, no. 5, pp. 945--958, 2009.
\end{thebibliography}
\fi
